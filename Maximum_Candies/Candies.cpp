...
class Solution {
public:
    int bfs(vector<int>graph[], int first, int size,vector<int>& status, vector<int>& candies, vector<vector<int>>& keys) {
    ...
    int maxCandies(vector<int>& status, vector<int>& candies, vector<vector<int>>& keys, vector<vector<int>>& containedBoxes, vector<int>& initialBoxes) {
        ...
        if(initialBoxes.size()>=0){
            for(int i = 0; i < initialBoxes.size(); ++i){
                total += bfs(graph, initialBoxes[i], status.size(),status,candies,keys);
                //cout <<  initialBoxes[i] << " total: " << total << endl;
            }
        }
        return total;
    }
...
int main() {
   ...
   // output = 1
    vector<int> status = {1, 1, 1};  // Status das caixas
    vector<int> candies = {100, 1, 100};  // Quantidade de doces nas caixas
    vector<vector<int>> keys = {{}, {0,2}, {}};  // Chaves que cada caixa contém
    vector<vector<int>> containedBoxes = {{}, {}, {}, {}, {}};  // Caixas que cada caixa contém
    vector<int> initialBoxes = {1};  // Caixas iniciais
    */

    // Exemplo de entrada de teste, output = 1
    ...
    vector<int> initialBoxes = {2,7,8,9,16,17,21,22,23,25,28,32,33,34,36,37,42,44,45,48};  // Caixas iniciais
